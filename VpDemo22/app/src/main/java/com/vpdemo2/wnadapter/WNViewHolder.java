...
public class WNViewHolder {
    ...
    public WNViewHolder(Context context, ViewGroup parent, int layoutId, int position, int type) {
        mViews = new SparseArray<View>();
        mPosition = position;
        mConvertView = LayoutInflater.from(context).inflate(layoutId, parent, false);
        mConvertView.setTag(this);
    ...
    public static WNViewHolder getViewHolder(Context context, View convertView, ViewGroup parent, int layoutId, int position, int type) {
        if (convertView == null) {
            return new WNViewHolder(context, parent, layoutId, position, type);
        } else {
            WNViewHolder holder = (WNViewHolder) convertView.getTag();
            return holder;
        }
